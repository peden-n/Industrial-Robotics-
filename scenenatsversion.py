...
class Environment:
    def __init__(self):
        ...
        self.env.launch(realtime=
                   True)  # set to False for faster-than-real-time simulation
        ...
        Boxbase = os.path.join(current_dir,"BoxBase.stl")
        ...
        Boxbase = geometry.Mesh(Boxbase, pose=SE3(0.0, 0.48, 0.5), color=(0.5, 0, 0, 1), scale=SCALE_TUPLE)
        # DonutBurnt = geometry.Mesh(DonutBurnt, pose=SE3(0.7, 0.5, 0.9).A, color=(1.0, 0.8, 0.6, 1), scale=(0.001, 0.001, 0.001))

        self.env.add(Stand1)
        self.env.add(Stand2)
        self.env.add(Stand3)
        self.env.add(TableLong)
        self.env.add(TableShort)
        self.env.add(Conveyor)
        self.env.add(Pallet)
        self.env.add(Walls)
        self.env.add(Boxbase)
        # env.add(DonutBurnt)
