...
def main():
    ...
    # Converted from the provided Link(...) definitions into DHLink(...)
    ...
    # Initial joint configuration (radians). If you want a different
    # starting pose adjust these values. Assumption: start at a mild pose
    # for visualization and testing.
    q_init = np.deg2rad([0.0, -30.0, 45.0, 0.0, 10.0, 0.0])
    q = q_init.copy()  # six joints

    # Do NOT create the cylinder skeleton visualization; we prefer using STL meshes only.
    # (If you want cylinder visuals, re-enable CylindricalDHRobotPlot here.)
    # Set robot to initial joint configuration for the teach GUI
    robot.q = q_init.copy()

    # compute script directory once for potential STL meshes
    script_dir = os.path.dirname(os.path.abspath(__file__))

    # Continuously toggle between teach GUI and Swift (or matplotlib) until Ctrl+C
    mode = "teach"
    ...
    try:
        while True:
            if mode == "teach":
                ...
                # teach() only supports pyplot backend, use that explicitly
                ...
                # Now launch Swift (or fallback) and apply the final pose to the simulation
                try:
                    # Launch a Swift environment and add the robot (preferred)
                    env = swift.Swift()
                    ...
                    for k in range(0, 7):
                        ...
                        if os.path.exists(stl_path):
                            try:
                                # set mesh color to blue and add only the STL meshes (no cylinder skeleton)
                                # standard blue: RGB(0,0,255) normalized to (0,0,1)
                                mesh = geometry.Mesh(
                                    stl_path,
                                    pose=SE3(),
                                    scale=(1.0, 1.0, 1.0),
                    ...
                    while True:
                        ...
                        # Update mesh poses to match robot forward kinematics if possible
                        try:
                        ...
                        # Swift environments expect periodic stepping
                        try:
                            env.step(0.05)
                        except Exception:
                            time.sleep(0.05)
